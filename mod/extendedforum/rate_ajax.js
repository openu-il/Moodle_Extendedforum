...
/// Javascript used to handle AJAX extendedforum ratings

/**
 * This function initializes all the stuff needed to have extendedforum ratings
 ...
 * menu found in he page (class = extendedforumpostratingmenu)
 * and prevents manual submission
 */
function add_menu_listeners(e) {

    /** hide the submit button */
    var submitbutton = YAHOO.util.Dom.get('extendedforumpostratingsubmit');
    ...
    var menus = YAHOO.util.Dom.getElementsByClassName('extendedforumpostratingmenu', 'select');
...
function perform_rate(e, menu) {
    ...
    var url = rate_ajax_config.wwwroot +  '/mod/extendedforum/rate_ajax.php?postid=' + menu.name + '&rate=' + menu.value + '&sesskey=' + rate_ajax_config.sesskey;
...
function display_response(menu, response) {
    ...
    var ratingsSpan = YAHOO.util.Dom.getFirstChildBy(ratingsDiv, function(el){return YAHOO.util.Dom.hasClass(el,'extendedforumpostratingtext');});

    /** span doesn't exist (first rate), add it, shouldn't happen ever but... fallback**/
    if (!ratingsSpan) {
        ratingsSpan = document.createElement('span');
        YAHOO.util.Dom.addClass(ratingsSpan, 'extendedforumpostratingtext');
